...
@dcs.route('/missions/old', methods=['GET', 'POST'])
def old_list_missions():
        player_makeup = {
            'jeremy': [
                'AJS37',
                'A-10C',
        ...
        #C:\\Users\\Tim\\Saved Games\\DCS.openbeta\\Missions
        search_path = 'misc'
        search_path = 'C:\\Users\\Tim\\Saved Games\\DCS.openbeta\\Missions'
        count = 0
        for mission in parser.list_missions(search_path):
            count += 1
            missions[mission] = parser.parse_mission(mission)
        print("Parsed {} missions".format(count))

        final_missions = {}

        for m_name, mission in missions.items():
            if mission['factions']['blue']['slot_count'] < len(players):
                continue
            meets_needs = True
            for player in players:
                if not list(set(
                        list(mission['factions']['blue']['aircraft'].keys())
                    ).intersection(
                        player_makeup[player]
                )):
                    meets_needs = False
            if meets_needs:
                mission['meets_filter'] = True
                final_missions[m_name] = mission
        print(missions)
